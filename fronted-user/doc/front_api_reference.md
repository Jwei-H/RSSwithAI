...
| sourceId | long | 否 | 指定 RSS 源内搜索（仅对 RSS 源生效；传入后优先于 `searchScope`） |
...
- 搜索关键词必填，不能为空
- 混合检索策略：
  1. **模糊匹配**：基于 query 本身进行模糊匹配，并使用 jieba TF-IDF 提取 top1 关键词做二次模糊召回
  2. **向量相似度**：将查询词向量化，与文章向量计算相似度，返回最多 50 条
  3. **并行召回**：模糊召回与向量召回并行执行
  4. **结果重排**：按相关性得分与时间衰减综合排序
- 如果向量生成失败，仅使用关键词模糊匹配
- 当 `searchScope=FAVORITE` 且当前用户没有收藏时，返回空列表
- 订阅页中，当用户处于某个 RSS 订阅上下文且传入 `sourceId` 时，仅在该 RSS 源内搜索
- 订阅页中，当用户处于 Topic 订阅上下文时，客户端默认使用 `searchScope=ALL`
