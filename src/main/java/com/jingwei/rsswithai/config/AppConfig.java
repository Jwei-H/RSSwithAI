...
public class AppConfig {
    ...
    private Double feedSimilarityThreshold = 0.3;
    ...
    private String trendsHotEventsReducePrompt = """
            你是一个全球热点聚合引擎。以下是从多个来源收集到的新闻事件片段。很多片段描述的是同一个事件，请进行语义识别、去重和合并。
            并根据事件被提及的频率（隐含在列表中重复出现的次数）以及事件本身的行业影响力（科技类可适当偏高），打一个热度分（1-10，10为最高）。
