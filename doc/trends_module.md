...
2. **热点事件榜单**：展示全网（或全局）最受关注的 Top 事件，按价值打分排序。
...
1. **Map 阶段（源内提取）**：
   - 遍历活跃 RSS 源，选取最近 4 天文章。
  - 若单个源文章数超过 30，则按每 30 篇拆分为多个独立 Map 子任务执行。
...
2. **Reduce 阶段（全局聚合）**：
  ...
  - 对同一 RSS 源产生的多个 Map 结果，Reduce 输入中的源名使用 `原始源名-partN`（如 `澎湃新闻-part1`）。
  - 对空事件列表的 RSS 源直接跳过，不参与 Reduce。
  - 调用 LLM 进行跨源语义去重、归并、打分（1-10分），最终输出最多 30 个热点事件。
3. **持久化**：
  - 词云与热点事件均采用**追加写入**（append-only），不覆盖历史记录。
  - 查询时统一读取指定维度的**最新一条记录**。

> 详细Prompt见`AppConfig.java`
