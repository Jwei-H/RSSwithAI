...
5. **收藏管理**：收藏/取消收藏文章，分页查看收藏列表（按发布时间倒序）。
...
1. **收藏文章**：校验文章是否存在，若已收藏则保持幂等；否则写入收藏关系。
2. **取消收藏**：根据 `userId` 与 `articleId` 删除收藏记录，若不存在则忽略。
3. **收藏列表**：按文章发布时间倒序分页返回收藏的文章摘要（`ArticleFeedDTO`）。