...
- **关键词分 ($S_{keyword}$)**: 命中得 1.0（权重 $W_{key}=0.6$，相较语义分适当降低，避免纯文本匹配主导排名）
- **时间衰减 ($Decay(t)$)**:  以**小时**为单位进行平滑衰减：`1.0 / (1.0 + hours_diff * 0.002)`
- **关键词提取门槛**: 仅当 query 长度 ≥ 4 时，才使用 TFIDF 提取 top1 关键词辅助召回；过短 query 直接使用原文搜索。
